...
class PCIScraper:
    ...
    def scrape_pci_concursos(self, max_concursos=30):
        ...
        try:
            # URLs prioritárias do PCI Concursos (incluindo últimas atualizações)
            urls_tentar = [
                f"{self.base_url}/ultimas/",  # NOVA URL com últimas atualizações
                f"{self.base_url}/",
    ...
    def extrair_concursos_pagina(self, html, url_base):
        ...
        # Seletores específicos para página de últimas atualizações
        if '/ultimas/' in url_base:
            logger.info("🎯 Processando página de últimas atualizações do PCI")
            # Buscar estruturas específicas da página de últimas
            seletores_ultimas = [
                'tr[class*="linha"]',      # Linhas da tabela de concursos
                'tr[class*="item"]',       # Items de concurso
                'div[class*="concurso"]',  # Divs de concurso
                'a[href*="/concurso/"]',   # Links diretos
                'table tr td a',           # Links em tabelas
            ]
            
            elementos_encontrados = []
            for seletor in seletores_ultimas:
    ...
    def extrair_dados_elemento(self, elemento):
        ...
        try:
            # Verificar se é uma linha de tabela (comum em /ultimas/)
            if elemento.name == 'tr':
                return self.extrair_dados_tabela(elemento)
    ...
    def extrair_dados_tabela(self, linha_tr):
        """Extrai dados específicos de uma linha de tabela (formato /ultimas/)"""
        try:
            colunas = linha_tr.find_all(['td', 'th'])
            if len(colunas) < 2:  # Muito poucas colunas
                return None
            ...
            titulo = textos_colunas[0] if textos_colunas else "Concurso"
            
            # Tentar identificar informações nas colunas
            orgao = ""
            local = ""
            data_info = ""
            
            for texto in textos_colunas:
                if any(palavra in texto.lower() for palavra in ['prefeitura', 'tribunal', 'polícia', 'ministério']):
                    orgao = texto
                elif any(estado in texto.upper() for estado in ['SP', 'RJ', 'MG', 'RS', 'PR', 'SC', 'BA', 'GO']):
                    local = texto
                elif re.search(r'\d{1,2}/\d{1,2}/\d{4}', texto):
                    data_info = texto
            
            dados = {
                'id': f"pci_tab_{hash(link or titulo) % 100000}",
                'titulo': self.limpar_texto(titulo),
                ...
                'urgencia': 'normal',
                'url': link,
                'fonte': 'PCI Concursos Real - Últimas',
                'tipo_documento': 'edital',
                'conteudo': self.limpar_texto(' '.join(textos_colunas))
            }
    ...
    def scrape_alternativo(self):
        ...
        try:
            # URLs alternativas para tentar (incluindo últimas atualizações)
            urls_alt = [
                f"{self.base_url}/ultimas/",      # Prioridade: últimas atualizações
                f"{self.base_url}/concursos/abertos",
